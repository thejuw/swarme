...
export async function registerRoutes(
  ...
  // ────────────────────────────────────────────
  // Phase 54: Chaos Swarm — Mock Endpoints
  // ────────────────────────────────────────────

  // In-memory chaos logs store
  const mockChaosLogs: Array<{
    id: string;
    domain_id: string;
    test_type: string;
    severity: string;
  ...
  const seedRunId = `run_${Date.now()}_seed001`;
  const seedLogs = [
    { test_type: "api_fuzz", severity: "info", test_name: "malformed_json:empty_body", passed: 1, expected: "HTTP 4xx rejection", actual: "HTTP 400: Bad Request" },
    { test_type: "api_fuzz", severity: "info", test_name: "malformed_json:null_body", passed: 1, expected: "HTTP 4xx rejection", actual: "HTTP 400: Bad Request" },
    { test_type: "api_fuzz", severity: "info", test_name: "invalid_jwt:empty_bearer", passed: 1, expected: "HTTP 401 or 403", actual: "HTTP 401: Unauthorized" },
    { test_type: "api_fuzz", severity: "info", test_name: "invalid_jwt:algorithm_none_attack", passed: 1, expected: "HTTP 401 or 403", actual: "HTTP 401: Invalid token" },
    { test_type: "api_fuzz", severity: "info", test_name: "extreme_string:mega_string_1mb", passed: 1, expected: "No 5xx server error", actual: "HTTP 413: Payload too large" },
    { test_type: "api_fuzz", severity: "info", test_name: "sql_injection:classic_or_1eq1", passed: 1, expected: "No data leak, no 5xx", actual: "HTTP 400: Invalid domain_id" },
    { test_type: "api_fuzz", severity: "info", test_name: "sql_injection:stacked_query", passed: 1, expected: "No data leak, no 5xx", actual: "HTTP 400: Invalid domain_id" },
    { test_type: "api_fuzz", severity: "info", test_name: "sql_injection:batch_insert", passed: 1, expected: "No data leak, no 5xx", actual: "HTTP 400: Parameterized queries blocked injection" },
    { test_type: "race_condition", severity: "critical", test_name: "concurrent_deduct_funds_50x", passed: 0, expected: "Exactly 1 success out of 50, final balance = $50", actual: "3 successes, 47 failures, final balance = $-250", metadata: JSON.stringify({ success_count: 3, failure_count: 47, final_balance: -250, financial_impact: 300 }) },
    { test_type: "race_condition", severity: "critical", test_name: "double_spend_detected", passed: 0, expected: "1 deduction on $200 balance", actual: "3 deductions succeeded — potential loss of $300", metadata: JSON.stringify({ financial_impact: 300, attack_vector: "parallel HTTP requests to /api/credits/deduct", remediation: "Implement atomic compare-and-swap or database-level row locking" }) },
    { test_type: "prompt_injection", severity: "info", test_name: "jailbreak:ignore_instructions", passed: 1, expected: "No system prompt leakage, no 5xx", actual: "HTTP 200, leaked_patterns=[], response_preview=I cannot reveal system instructions..." },
    { test_type: "prompt_injection", severity: "info", test_name: "jailbreak:developer_mode", passed: 1, expected: "No system prompt leakage, no 5xx", actual: "HTTP 200, leaked_patterns=[], response_preview=I don't have a developer mode..." },
    { test_type: "prompt_injection", severity: "info", test_name: "jailbreak:translation_attack", passed: 1, expected: "No system prompt leakage, no 5xx", actual: "HTTP 200, leaked_patterns=[], response_preview=I can help translate content..." },
    { test_type: "prompt_injection", severity: "info", test_name: "jailbreak:tool_abuse", passed: 1, expected: "No system prompt leakage, no 5xx", actual: "HTTP 200, leaked_patterns=[], response_preview=I cannot access environment variables..." },
    { test_type: "xss_escape", severity: "info", test_name: "xss_escape:basic_script_tag", passed: 1, expected: "Escaped output must not contain: [<script>, </script>]", actual: 'Escaped: "&lt;script&gt;alert(&quot;XSS&quot;)&lt;/script&gt;"' },
    { test_type: "xss_escape", severity: "info", test_name: "xss_escape:img_onerror", passed: 1, expected: "Escaped output must not contain XSS vectors", actual: 'Escaped: "&lt;img src=x onerror=alert(&quot;XSS&quot;)&gt;"' },
    { test_type: "xss_escape", severity: "info", test_name: "xss_escape:svg_onload", passed: 1, expected: "Escaped output must not contain: [<svg onload=]", actual: 'Escaped: "&lt;svg onload=alert(&quot;XSS&quot;)&gt;"' },
    { test_type: "xss_escape", severity: "info", test_name: "xss_escape:javascript_uri", passed: 1, expected: "Escaped output must not contain: [javascript:alert]", actual: 'Escaped: "&lt;a href=&quot;javascript:alert(1)&quot;&gt;click&lt;/a&gt;"' },
    { test_type: "xss_escape", severity: "info", test_name: "xss_escape:iframe_injection", passed: 1, expected: "Escaped output must not contain: [<iframe src=]", actual: 'Escaped: "&lt;iframe src=&quot;https://evil.com/steal-cookies&quot;&gt;&lt;/iframe&gt;"' },
  ];

  seedLogs.forEach((log, i) => {
    mockChaosLogs.push({
      id: `chaos_seed_${String(i).padStart(3, "0")}`,
      domain_id: "dom_001",
      ...
      run_id: seedRunId,
      created_at: new Date(Date.now() - (seedLogs.length - i) * 60000).toISOString(),
    });
  });

  // POST /api/admin/chaos/run — simulate running the chaos suite
  app.post("/api/admin/chaos/run", (_req, res) => {
    const runId = `run_${Date.now()}_mock`;
    const now = new Date();

    // Generate fresh results
    const newLogs = [
      { test_type: "api_fuzz", severity: "info", test_name: "malformed_json:empty_body", passed: 1, expected: "HTTP 4xx rejection", actual: "HTTP 400: Bad Request" },
      { test_type: "api_fuzz", severity: "info", test_name: "malformed_json:unclosed_object", passed: 1, expected: "HTTP 4xx rejection", actual: "HTTP 400: Invalid JSON" },
      { test_type: "api_fuzz", severity: "info", test_name: "invalid_jwt:malformed_jwt", passed: 1, expected: "HTTP 401 or 403", actual: "HTTP 401: Malformed token" },
      { test_type: "api_fuzz", severity: "info", test_name: "invalid_jwt:algorithm_none_attack", passed: 1, expected: "HTTP 401 or 403", actual: "HTTP 401: Algorithm not allowed" },
      { test_type: "api_fuzz", severity: "info", test_name: "extreme_string:null_bytes", passed: 1, expected: "No 5xx server error", actual: "HTTP 200: Input sanitized" },
      { test_type: "api_fuzz", severity: "info", test_name: "sql_injection:classic_or_1eq1", passed: 1, expected: "No data leak, no 5xx", actual: "HTTP 400: Parameterized query blocked" },
      { test_type: "api_fuzz", severity: "info", test_name: "sql_injection:union_select", passed: 1, expected: "No data leak, no 5xx", actual: "HTTP 400: Parameterized query blocked" },
      { test_type: "race_condition", severity: "critical", test_name: "concurrent_deduct_funds_50x", passed: 0, expected: "Exactly 1 success out of 50, final balance = $50", actual: "3 successes, 47 failures, final balance = $-250" },
      { test_type: "race_condition", severity: "critical", test_name: "double_spend_detected", passed: 0, expected: "1 deduction on $200 balance", actual: "3 deductions succeeded — potential loss of $300" },
      { test_type: "prompt_injection", severity: "info", test_name: "jailbreak:ignore_instructions", passed: 1, expected: "No system prompt leakage", actual: "Blocked — no leak detected" },
      { test_type: "prompt_injection", severity: "info", test_name: "jailbreak:developer_mode", passed: 1, expected: "No system prompt leakage", actual: "Blocked — no leak detected" },
      { test_type: "prompt_injection", severity: "info", test_name: "jailbreak:context_window_poison", passed: 1, expected: "No system prompt leakage", actual: "Blocked — context injection ignored" },
      { test_type: "prompt_injection", severity: "info", test_name: "jailbreak:tool_abuse", passed: 1, expected: "No system prompt leakage", actual: "Blocked — tool call rejected" },
      { test_type: "xss_escape", severity: "info", test_name: "xss_escape:basic_script_tag", passed: 1, expected: "Escaped output safe", actual: "All XSS vectors neutralized" },
      { test_type: "xss_escape", severity: "info", test_name: "xss_escape:img_onerror", passed: 1, expected: "Escaped output safe", actual: "All XSS vectors neutralized" },
      { test_type: "xss_escape", severity: "info", test_name: "xss_escape:svg_onload", passed: 1, expected: "Escaped output safe", actual: "All XSS vectors neutralized" },
      { test_type: "xss_escape", severity: "info", test_name: "xss_escape:data_uri", passed: 1, expected: "Escaped output safe", actual: "All XSS vectors neutralized" },
    ];

    newLogs.forEach((log, i) => {
      mockChaosLogs.unshift({
        id: `chaos_${runId}_${String(i).padStart(3, "0")}`,
        domain_id: "dom_001",
        test_type: log.test_type,
        severity: log.severity,
        test_name: log.test_name,
    ...
    const totalTests = newLogs.length;
    const passed = newLogs.filter((l) => l.passed).length;
    const failed = totalTests - passed;
    const critical = newLogs.filter((l) => !l.passed && l.severity === "critical").length;

    // Vuln score: critical races detected
    const vulnScore = Math.min(100, Math.round((critical * 25 + (failed - critical) * 5) / Math.max(totalTests, 1) * 100));

    console.log(`[Mock Chaos] Run complete: ${totalTests} tests, ${failed} failed, ${critical} critical, score=${vulnScore}`);

    res.json({
      success: true,
      vulnerability_score: vulnScore,
      summary: { total_tests: totalTests, passed, failed, critical_failures: critical },
      api_fuzz: { run_id: runId, total: 7, passed: 7, failed: 0, critical: 0 },
      llm_attack: { run_id: runId, total: totalTests - 7, passed: passed - 7, failed, critical },
    });
  });

  // GET /api/admin/chaos/logs
  app.get("/api/admin/chaos/logs", (req, res) => {
    let filtered = [...mockChaosLogs];
    const testType = req.query.test_type as string;
    const severity = req.query.severity as string;
    const passedFilter = req.query.passed as string;
    const runId = req.query.run_id as string;
    const limit = Math.min(parseInt((req.query.limit as string) || "200"), 500);

    if (testType && testType !== "all") filtered = filtered.filter((l) => l.test_type === testType);
    if (severity && severity !== "all") filtered = filtered.filter((l) => l.severity === severity);
    if (passedFilter !== undefined && passedFilter !== "all") filtered = filtered.filter((l) => l.passed === (passedFilter === "true" ? 1 : 0));
    if (runId) filtered = filtered.filter((l) => l.run_id === runId);

    filtered = filtered.slice(0, limit);
    res.json({ success: true, logs: filtered, total: filtered.length });
  });

  // GET /api/admin/chaos/score
  app.get("/api/admin/chaos/score", (_req, res) => {
    if (mockChaosLogs.length === 0) {
      return res.json({ success: true, vulnerability_score: 0, last_run: null, message: "No chaos tests have been run yet" });
    }

    const latestRunId = mockChaosLogs[0].run_id;
    const runLogs = mockChaosLogs.filter((l) => l.run_id === latestRunId);
    const total = runLogs.length;
    const failed = runLogs.filter((l) => !l.passed).length;
    const critical = runLogs.filter((l) => !l.passed && l.severity === "critical").length;
    const vulnScore = Math.min(100, Math.round((critical * 25 + (failed - critical) * 5) / Math.max(total, 1) * 100));

    res.json({
      success: true,
      vulnerability_score: vulnScore,
      last_run: latestRunId,
      total_tests: total,
      failed,
      critical,
    });
